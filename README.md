...
Run `make spelling` to enforce en-GB-oxendict prose spelling. The target
regenerates `typos.toml` from the live shared estate dictionary and the narrow
repository policy in `typos.local.toml`, then checks the tracked Markdown
files. Edit the local policy rather than generated entries.
